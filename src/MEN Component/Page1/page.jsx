  ...
  return (
    <section className="relative w-full min-h-screen bg-[#f5f5f7] overflow-hidden flex items-center justify-center py-20 md:py-0">
      ...
      <div className="relative z-10 w-full h-full grid grid-cols-1 md:grid-cols-12 px-6 md:px-10">
        ...
        {/* LEFT COLUMN: Metadata - Hidden on small mobile, visible on tablet+ */}
        <div className="hidden md:flex md:col-span-3 flex-col justify-end pb-20 border-r border-black/5">
          <motion.div 
            initial={{ opacity: 0, x: -20 }}
            animate={{ opacity: 1, x: 0 }}
        ...
        {/* CENTER COLUMN: The Visual Hook */}
        <div className="col-span-1 md:col-span-6 flex flex-col items-center justify-center relative px-4 md:px-0">
          <motion.div 
            initial={{ scale: 0.9, opacity: 0 }}
            animate={{ scale: 1, opacity: 1 }}
            transition={{ duration: 1.5, ease: [0.16, 1, 0.3, 1] }}
            className="relative w-full aspect-[4/5] md:aspect-auto md:w-[80%] md:h-[75vh] group"
          >
            {/* The Image */}
            <img 
              src="https://i.postimg.cc/JzRFdhY4/70ce13a05c86ca70b4e39798d114cef0.jpg" 
              alt="Men's Collection" 
              className="w-full h-full object-cover grayscale hover:grayscale-0 transition-all duration-1000 shadow-2xl md:shadow-none"
            />
            
            {/* Overlay Text */}
            <div className="absolute inset-0 flex flex-col items-center justify-center mix-blend-difference pointer-events-none">
              <h1 className="text-white text-[18vw] md:text-[8vw] font-light tracking-tighter leading-none text-center">
                NEZIN <br /> <span className="font-serif italic">MEN</span>
              </h1>
            </div>
          </motion.div>

          {/* Mobile-Only Metadata (Brief) */}
          <div className="flex md:hidden flex-col items-center mt-8 gap-2">
             <h3 className="text-xl font-light tracking-tighter text-black">Architecture for the modern man.</h3>
             <span className="text-[8px] tracking-[0.3em] text-black/40 uppercase">Fall / Winter 2026</span>
          </div>

          {/* Scroll Indicator */}
          <div className="absolute -bottom-12 md:bottom-10 flex flex-col items-center gap-4">
            <span className="text-[8px] md:text-[9px] tracking-[0.5em] text-black/40 uppercase">Scroll to explore</span>
            <motion.div 
              animate={{ y: [0, 10, 0] }}
              transition={{ repeat: Infinity, duration: 2 }}
              className="w-[1px] h-8 md:h-10 bg-black/20"
            />
          </div>
        </div>
